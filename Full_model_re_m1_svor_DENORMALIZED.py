...
# Splitting data for two sets for training
X_train, X_test, y_train_idx, y_test_idx = train_test_split(data[features], range(len(data)), test_size=0.2, random_state=42)

# Splitting data for two sets FOR TARGET DATA(re_m1_svor) in order to have target in raw format, but other data
# in normalized format
y_train = data[target].iloc[y_train_idx]
y_test = data[target].iloc[y_test_idx]
...
# Test with randomly picked normalized data
...
print('Predicted re_m1_svor:', y_new_pred)
